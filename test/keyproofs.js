...
contract('KeyProofs', function(accounts) {
  ...
  it("allows creating one challenge per-uuid", async function() {
    ...
    let meta = await KeyProofs.at("0xc635bd5f6b0f66b1b70faa7fe95d219a5bf8e446");
  ...
  it("returns response information ", async function() {
    ...
    let submitProofTx = await meta.submitProof(uuid, accounts[1], metaProof.hash, metaProof.v, metaProof.r, metaProof.s, {from: accounts[1]});
  ...
  it("does not allow submitting proofs without a valid challenge uuid", async function() {
    ...
    let submitProofTx = await meta.submitProof(uuid, reportedSigningAccount, 
      metaProof.hash, metaProof.v, metaProof.r, metaProof.s, {from: accounts[1]}).catch(function() {});
  ...
  it("does not allow submitting proofs with invalid signature", async function() {
    ...
    let submitProofTx = await meta.submitProof(uuid, reportedSigningAccount, metaProof.hash, metaProof.v, metaProof.r, metaProof.s, {from: accounts[1]}).catch(function() {});
  ...
  it("allows submitting valid proofs exactly once", async function() {
    ...
    let submitProofTxA = await meta.submitProof(uuid, accounts[1], 
      metaProof.hash, metaProof.v, metaProof.r, metaProof.s, {from: accounts[1]});
    let submitProofTxB = await meta.submitProof(uuid, accounts[1], 
      metaProof.hash, metaProof.v, metaProof.r, metaProof.s, {from: accounts[1]}).catch(function() {});
